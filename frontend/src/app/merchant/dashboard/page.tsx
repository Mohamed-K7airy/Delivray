...
export default function MerchantDashboard() {
  ...
  const getStatusDisplay = (status: string) => {
    switch(status.toLowerCase()) {
      case 'pending': return { label: 'PENDING', color: 'bg-gray-100 text-gray-500 border-gray-200' };
      case 'accepted': return { label: 'ACCEPTED', color: 'bg-green-50 text-green-500 border-green-100' };
      case 'preparing': return { label: 'PREPARING', color: 'bg-orange-50 text-[#FF5A3C] border-orange-100' };
      case 'ready_for_pickup': return { label: 'READY', color: 'bg-blue-50 text-blue-500 border-blue-100' };
      case 'delivering': return { label: 'SHIPPING', color: 'bg-purple-50 text-purple-500 border-purple-100' };
      case 'completed': return { label: 'COMPLETED', color: 'bg-green-50 text-green-600 border-green-200' };
      default: return { label: status.toUpperCase(), color: 'bg-gray-100 text-gray-400' };
    }
  };
      ...
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
         <div>
            ...
            <div className="flex items-center gap-4">
               ...
               <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Real-time gateway connected</span>
            </div>
         </div>
         <div className="flex items-center gap-4">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-10 lg:gap-14 items-start">
         ...
         <div className="xl:col-span-8 space-y-10">
            <div className="bg-white rounded-[3rem] p-10 border border-gray-100 shadow-[0_10px_40px_rgba(0,0,0,0.02)]">
               <div className="flex items-center justify-between mb-10">
                  ...
                  <button 
                    ...
                    className="text-[10px] font-black text-[#FF5A3C] uppercase tracking-widest hover:underline"
                  >
                    View All Queue
                  </button>
               </div>
               ...
               <div className="space-y-6">
                  {loading ? (
                    ...
                    <motion.div 
                      ...
                      className="group bg-[#F8F8F8] hover:bg-white hover:shadow-xl hover:shadow-black/5 rounded-[2rem] p-6 lg:p-8 flex items-center justify-between border border-transparent hover:border-gray-100 transition-all"
                    >
                       <div className="flex items-center gap-6 lg:gap-10">
                          <div className={`w-16 h-16 rounded-2xl flex items-center justify-center text-3xl shadow-sm border border-white transition-transform group-hover:scale-110 duration-500 bg-white`}>
                             {idx === 0 ? '🍔' : idx === 1 ? '🍕' : '🥐'}
                          </div>
                          <div>
                             <div className="flex flex-wrap items-center gap-4 mb-2">
                                <span className="text-lg font-black text-[#0A0A0A] tracking-tight">{order.id.slice(0, 8).toUpperCase()} — {firstItem}</span>
                                <span className={`px-4 py-1.5 rounded-full text-[9px] font-black uppercase tracking-widest border ${statusInfo.color}`}>
                                   {statusInfo.label}
                                </span>
                             </div>
                             <p className="text-xs font-bold text-gray-400">
                                Ordered by <span className="text-[#0A0A0A]">{order.customer?.users?.name || 'Operational Node'}</span> • {new Date(order.created_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                             </p>
                          </div>
                       </div>
                       <div className="flex flex-col sm:flex-row items-center gap-6 lg:gap-8">
                          <div className="text-right hidden sm:block">
                             <p className="text-[10px] font-black uppercase tracking-widest mb-1 text-gray-400">Action Required</p>
                             <p className="text-xs font-bold text-gray-500 italic">#{order.id.slice(0,4)} Registry</p>
                          </div>
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <button 
                              onClick={() => router.push('/merchant/orders')}
                              className="w-12 h-12 rounded-2xl bg-white flex items-center justify-center text-gray-300 group-hover:text-[#FF5A3C] group-hover:bg-[#FF5A3C]/5 border border-gray-50 transition-all active:scale-90"
                            >
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8 lg:gap-10">
               {[
                 { label: 'TOTAL SALES (TODAY)', value: `$${(orders.reduce((acc, o) => acc + Number(o.total_price), 0)).toFixed(2)}`, sub: '+Real-time calculation', icon: <DollarSign size={20} />, color: '#FF5A3C' },
                 { label: 'SUCCESS RATE', value: '100%', sub: 'Target: 95%', icon: <TrendingUp size={20} />, color: '#0A0A0A' },
                 { label: 'PENDING TASKS', value: orders.filter(o => o.status === 'pending').length, sub: 'ACTION NEEDED', icon: <Tag size={20} />, color: '#0A0A0A' },
               ].map((stat, idx) => (
                 <motion.div 
                    key={stat.label}
            ...
            >
               <div className="relative z-10 flex flex-col md:flex-row justify-between items-center gap-10">
                  <div className="max-w-md">
                     <span className="bg-[#FF5A3C] text-white px-4 py-1.5 rounded-full text-[9px] font-black uppercase tracking-widest mb-6 inline-block">SYSTEM INSIGHT</span>
                     ...
                     <button 
                       ...
                       className="bg-white text-[#0A0A0A] px-10 py-4 rounded-xl font-black uppercase tracking-widest text-[10px] hover:bg-gray-100 transition-all flex items-center gap-3"
                     >
                        View Fleet Map
                     </button>
                  </div>
