...
const STATUS_CONFIG: Record<string, { label: string; pill: string }> = {
  ...
  preparing:        { label: 'Preparing',  pill: 'bg-[#fef3f2] text-[#d97757] border-[#fee2e2]' },
...
export default function MerchantOrders() {
  ...
  const statCards = [
    { label: "Today's Orders", value: stats.totalOrdersToday, icon: <ShoppingBag size={18} />, accent: '#d97757', bg: '#fef3f2' },
    { label: 'Pending',        value: stats.pendingOrders,    icon: <Timer size={18} />,       accent: '#d97757', bg: '#fef3f2' },
  ...
  return (
    <div className="space-y-6 pb-24">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-3xl font-black text-[#111111] tracking-tighter">Orders</h1>
        ...
        <div className="flex items-center gap-3">
          <div className="relative">
            ...
            <input
              ...
              className="h-10 pl-9 pr-4 bg-white border border-gray-100 rounded-xl text-xs font-bold text-[#111111] placeholder:text-gray-300 outline-none focus:border-[#d97757] transition-all w-56"
            />
          </div>
          <button className="h-10 px-4 bg-white border border-gray-100 rounded-xl text-[10px] font-black uppercase tracking-wider text-[#888888] flex items-center gap-2 hover:bg-gray-50 transition-all">
      ...
      <div className="flex items-center gap-2 overflow-x-auto pb-1 no-scrollbar">
        {FILTER_TABS.map(tab => (
          <button
            ...
            className={`h-8 px-4 rounded-full text-[10px] font-black uppercase tracking-wider whitespace-nowrap shrink-0 transition-all border ${
              activeFilter === tab
        ...
        <span className="ml-2 text-[10px] font-black text-[#888888] shrink-0">{filteredOrders.length} orders</span>
      </div>
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-6 py-4">
                          <span className="text-xs font-black text-[#d97757] tracking-tight">
                        ...
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-3">
                            <div className="w-8 h-8 rounded-full bg-[#fef3f2] flex items-center justify-center text-[10px] font-black text-[#d97757] border border-[#fee2e2]">
                        ...
                        <td className="px-6 py-4">
                          <span className="text-xs font-medium text-[#555555] max-w-[180px] block truncate">
                            ...
                            {order.order_items?.length > 1 && <span className="text-[#d97757]"> +{order.order_items.length - 1}</span>}
                          </span>
                        </td>
                        ...
                        {/* Status Pill */}
                        <td className="px-6 py-4">
                          <span className={`inline-flex items-center px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest border ${cfg.pill}`}>
                            {cfg.label}
                          </span>
                        </td>
                        ...
                        <td className="px-6 py-4 text-right">
                          {isTerminal ? (
                            <span className="h-9 px-3 rounded-lg text-[9px] font-black uppercase tracking-wider bg-gray-50 text-gray-300 border border-gray-100 inline-flex items-center">
                            ...
                            <select
                              ...
                              className="h-9 pl-3 pr-7 rounded-lg text-[9px] font-black uppercase tracking-wider bg-[#111111] text-white border-0 outline-none cursor-pointer appearance-none hover:bg-[#333] transition-all"
        ...
        {filteredOrders.length > 0 && (
          <div className="px-6 py-4 bg-[#fafafa] border-t border-gray-50 flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              <button className="w-8 h-8 flex items-center justify-center bg-[#d97757] text-white rounded-lg text-[10px] font-black">1</button>
              <button className="w-8 h-8 flex items-center justify-center bg-white border border-gray-100 rounded-lg text-gray-300 hover:text-[#111111] transition-all">
