      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-4">
        {statCards.map((s, i) => (
          <motion.div key={s.label} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.06 }}
            className="bg-white rounded-xl sm:rounded-2xl p-3 sm:p-5 border border-gray-100 shadow-sm flex flex-col sm:flex-row items-start sm:items-center gap-2 sm:gap-4 group hover:shadow-md transition-all"
          >
            <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-lg sm:rounded-xl flex items-center justify-center shrink-0" style={{ backgroundColor: s.bg, color: s.accent }}>
              {s.icon}
            </div>
            <div>
              <p className="text-lg sm:text-xl font-bold text-[#111111] tracking-tighter leading-none truncate">{s.value}</p>
              <p className="text-[8px] sm:text-[9px] font-bold text-[#888888] uppercase tracking-widest mt-1 truncate">{s.label}</p>
            </div>
          </motion.div>
        ))}
      </div>
