...
const PlaceholderPage = ({ title, icon: Icon, description }: { title: string, icon: any, description: string }) => (
  <div className="max-w-4xl mx-auto py-20 text-center space-y-12">
    <motion.div 
      ...
      className="w-32 h-32 sm:w-48 sm:h-48 bg-white/5 rounded-3xl sm:rounded-[4rem] border border-white/5 flex items-center justify-center mx-auto group shadow-2xl"
    >
      <Icon size={80} className="text-gray-700 group-hover:text-[#ff8564] group-hover:scale-110 transition-all duration-700" />
    </motion.div>
    
    <div className="space-y-4">
      <h1 className="text-3xl sm:text-6xl font-black tracking-tighter uppercase">{title}</h1>
      <p className="text-gray-500 text-base sm:text-xl font-bold max-w-lg mx-auto leading-relaxed">{description}</p>
    </div>
