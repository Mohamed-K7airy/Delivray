        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
          ...
          <aside className="lg:col-span-4 xl:col-span-3 space-y-8">
             <motion.div 
               ...
               className="bg-white rounded-2xl p-10 border border-gray-100 shadow-md text-center relative overflow-hidden"
             >
                {/* Visual Accent */}
                <div className="absolute top-0 right-0 w-24 h-24 bg-[#d97757]/5 rounded-full -mr-12 -mt-12" />
                
                <div className="relative inline-block mb-8 group/avatar">
                   <div 
                      onClick={() => toast.info('Avatar upload coming soon!')}
                      className="w-32 h-32 rounded-full border-4 border-gray-50 shadow-lg overflow-hidden bg-gray-100 cursor-pointer relative"
                   >
                      <img 
                ...
                <h2 className="text-3xl font-black text-[#111111] tracking-tighter mb-2">{user?.name || 'John Doe'}</h2>
                <p className="text-[#888888] font-bold text-xs mb-6">{user?.phone || '+1(555) 012-3456'}</p>
                
                <span className="inline-flex items-center bg-[#fef3f2] text-[#d97757] px-5 py-2 rounded-full text-[9px] font-black uppercase tracking-widest border border-[#fee2e2] mb-10">
                   ● PREMIUM MEMBER
                </span>
                 <div className="space-y-3">
                   {[
                     ...
                     <button 
                       ...
                       className="w-full h-16 bg-[#f9f9f9] hover:bg-gray-100 rounded-xl p-5 flex items-center justify-between group transition-all border border-transparent hover:border-gray-200"
                     >
                        <div className="flex items-center gap-4 text-[#111111]">
                           <span className="text-[#888888] group-hover:text-[#d97757] transition-colors">{item.icon}</span>
                           <span className="text-[11px] font-black uppercase tracking-widest leading-none">{item.label}</span>
                        </div>
                        <ChevronRight size={16} className="text-gray-300 group-hover:text-[#111111] transition-colors" />
                     </button>
                   ))}
                </div>
             </motion.div>
             ...
             <motion.div
               ...
               className="bg-[#111111] rounded-2xl p-10 text-white relative overflow-hidden group shadow-lg"
             >
                <div className="absolute bottom-0 right-0 opacity-10 group-hover:opacity-20 transition-opacity">
                   <LifeBuoy size={120} />
                </div>
                <h4 className="text-2xl font-black tracking-tighter mb-4 relative z-10">Need help?</h4>
                <p className="text-[#888888] text-xs font-bold leading-relaxed mb-8 relative z-10">
                   Our 24/7 support team is always here to assist you with your orders.
                </p>
                <button className="h-14 bg-white text-[#111111] rounded-xl px-8 text-[11px] font-black uppercase tracking-widest hover:bg-gray-100 transition-all relative z-10">
                   Contact Support
                </button>
             </motion.div>
          ...
          <main className="lg:col-span-8 xl:col-span-9 space-y-10">
             <header>
                <p className="text-[11px] font-black text-[#d97757] uppercase tracking-[0.3em] mb-2 font-black italic">YOUR JOURNEY</p>
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                   <h1 className="text-5xl lg:text-6xl font-black text-[#111111] tracking-tighter">Recent Orders</h1>
                   <button className="h-14 bg-white border border-gray-100 rounded-xl px-6 flex items-center gap-4 text-[#111111] hover:bg-gray-50 transition-all shadow-sm">
                      <Filter size={18} />
                ...
                <AnimatePresence>
                   {orders.map((order, idx) => (
                     <motion.div 
                       ...
                       className="bg-white rounded-2xl p-6 lg:p-8 border border-gray-100 shadow-md flex flex-col md:flex-row items-center gap-8 group hover:border-gray-200 transition-all"
                     >
                        {/* Order Image */}
                        <div className="w-28 h-28 lg:w-32 lg:h-32 rounded-xl overflow-hidden bg-gray-50 shrink-0 border border-gray-100">
                           <img 
                        ...
                        <div className="flex-1 w-full text-center md:text-left space-y-4">
                           <div className="flex flex-col sm:flex-row justify-between items-center sm:items-start gap-4">
                              <div>
                                 <h3 className="text-2xl font-black text-[#111111] tracking-tighter mb-1">{order.restaurant || 'The Burger Collective'}</h3>
                              ...
                              <span className={`px-4 py-1.5 rounded-full text-[9px] font-black uppercase tracking-widest border ${
                                order.status === 'In Progress' || order.status === 'DELIVERING' 
                                  ...
                                  : 'bg-gray-50 text-[#888888] border-gray-100'
                              }`}>
                                 ● {order.status}
                              </span>
                           </div>

                           <div className="flex flex-wrap items-center justify-center md:justify-start gap-8 text-[11px] font-black text-[#888888] uppercase tracking-widest">
                              ...
                              <div className="flex items-center gap-2">
                                 ...
                                 <span className="text-[#111111]">${Number(order.price).toFixed(2)}</span>
                              </div>
                           </div>
                        </div>
                           ...
                           {order.status === 'In Progress' || order.status === 'DELIVERING' || order.status === 'PENDING' ? (
                             <>
                               <button 
                                 ...
                                 className="flex-1 md:flex-none h-16 bg-[#d97757] text-white rounded-xl px-10 text-[10px] font-black uppercase tracking-widest flex items-center justify-center gap-3 shadow-md hover:bg-[#c2654a] transition-all"
                               >
                               ...
                               <button 
                                 ...
                                 className="h-16 w-16 bg-[#f9f9f9] text-[#888888] hover:text-[#111111] rounded-xl flex items-center justify-center border border-gray-100 transition-all"
                               >
                                  <Activity size={20} />
                                </button>
                             </>
                             ...
                             <>
                               <button 
                                 ...
                                 className="flex-1 md:flex-none h-16 bg-[#f9f9f9] hover:bg-gray-100 text-[#111111] rounded-xl px-10 text-[10px] font-black uppercase tracking-widest flex items-center justify-center gap-3 transition-all"
                               >
                                  <Box size={16} />
                                  Order Again
                               </button>
                               <button 
                                 ...
                                 className="h-16 bg-white border border-gray-100 hover:border-gray-200 text-[#111111] rounded-xl px-8 text-[10px] font-black uppercase tracking-widest transition-all"
                               >
                                  View Receipt
                               </button>
                             </>
