  ...
  if (loading) {
    return (
      <div className="max-w-7xl mx-auto px-4 py-10">
        <div className="animate-pulse bg-white/10 h-64 rounded-[2rem] w-full mb-10"></div>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {[1,2,3,4,5,6].map(i => (
             <div key={i} className="animate-pulse bg-white h-40 rounded-[2rem] p-5 shadow-sm border border-transparent"></div>
          ))}
        </div>
      </div>
    );
    ...
    <div className="min-h-screen bg-[#111111] text-white">
      ...
      <nav className="h-16 border-b border-white/5 bg-[#262624]/80 backdrop-blur-xl flex items-center justify-between px-6 sticky top-0 z-50">
        <div className="flex items-center space-x-6">
          ...
          <div className="text-primary font-black text-xl tracking-tight uppercase italic">Neon Nocturne</div>
        </div>
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8 py-4 md:py-6">
        ...
        <motion.div 
          ...
          className="relative bg-[#262624] rounded-[2.5rem] p-8 md:p-14 mb-10 shadow-[0_40px_80px_-20px_rgba(0,0,0,0.8)] overflow-hidden flex flex-col justify-end min-h-[450px]"
        >
          ...
          <div className="relative z-20">
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-8">
              <div>
                <h1 className="text-5xl sm:text-7xl md:text-8xl font-black mb-10 tracking-tight leading-none uppercase">
                  {store.name}
                </h1>
                
                <div className="flex flex-wrap items-center gap-10">
                  <div className="flex items-center group cursor-help">
                    <Star size={20} className="text-primary fill-primary mr-3 group-hover:scale-125 transition-transform"/> 
                    <div>
                  ...
                  <div className="flex items-center group">
                    <MapPin size={20} className="text-primary mr-3 group-hover:translate-y-[-2px] transition-transform"/> 
                    <div className="text-lg font-black leading-none">1.2 miles away</div>
                  </div>

                  <div className="flex items-center group">
                    <Clock size={20} className="text-primary mr-3 group-hover:rotate-12 transition-transform"/> 
                    <div className="text-lg font-black leading-none">20-30 min</div>
                  </div>
                </div>
              </div>

              <button className="bg-white/5 backdrop-blur-3xl px-8 py-4 rounded-[1.5rem] border border-white/5 flex items-center space-x-3 hover:bg-white/10 transition-all font-black uppercase tracking-widest text-[11px] h-fit self-start md:self-end">
        ...
        <div className="flex items-center space-x-4 mb-12 overflow-x-auto pb-4 no-scrollbar">
          {categories.map((cat) => (
            <button
              ...
              className={`px-8 py-4 rounded-full text-[11px] font-black uppercase tracking-[0.2em] transition-all whitespace-nowrap shadow-sm
          ...
          store.products.map(product => {
            ...
            return (
              <motion.div 
                ...
                key={product.id} 
                className="bg-[#262624]/60 backdrop-blur-3xl p-7 rounded-[3rem] border border-white/5 hover:border-primary/30 transition-all flex items-center justify-between group h-fit shadow-2xl shadow-black/40"
              >
                <div className="flex-1 pr-4">
                  <div className="mb-1">
                     <h3 className="text-2xl font-extrabold text-white group-hover:text-primary transition-all duration-300">
                       {product.name} <span className="text-gray-500 font-bold text-sm ml-2">/ {product.name.includes(' ') ? 'صنف' : 'منتج'}</span>
                     </h3>
                  </div>
                  <p className="text-gray-500 text-[11px] leading-relaxed mb-8 line-clamp-2 font-medium">{product.description || 'Delicious freshly made item prepared with premium ingredients.'}</p>
                ...
                <div className="w-32 h-32 bg-gradient-to-br from-white/5 to-transparent rounded-[2.5rem] overflow-hidden relative border border-white/10 shrink-0 p-2 shadow-inner">
                   <img 
      ...
      <AnimatePresence>
        {items.length > 0 && (
          ...
          >
            <Link 
              href="/cart"
              className="bg-[#f08c6e] text-white pl-8 pr-12 py-6 rounded-full font-black uppercase tracking-[0.2em] shadow-[0_25px_60px_-15px_rgba(240,140,110,0.6)] flex items-center space-x-6 hover:scale-105 active:scale-95 transition-all group"
            >
               <ShoppingBag size={24} strokeWidth={3} className="text-white" />
